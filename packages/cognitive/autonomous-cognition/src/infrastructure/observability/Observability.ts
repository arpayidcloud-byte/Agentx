...
export class ExecutionMetricsCollector {
  ...
  record(name: string, value: number, tags: Record<string, string> = {}): MetricPoint {
    const point: MetricPoint = Object.freeze({ name, value, timestamp: new Date(), tags: { ...tags } });
  ...
  aggregate(name: string): { sum: number; count: number; avg: number; max: number; min: number } {
    ...
    return { sum, count: values.length, avg: sum / values.length, max: Math.max(...values), min: Math.min(...values) };
  }
...
export class ExecutionAuditManager {
  ...
  log(traceId: string, goalId: string, action: string, metadata: Record<string, unknown>): AuditEntry {
    const entryId = `ea-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
    const checksum = createHash('sha256').update(JSON.stringify({ traceId, goalId, action, metadata })).digest('hex');
    const entry: AuditEntry = Object.freeze({
      entryId, traceId, goalId, action, metadata: { ...metadata }, timestamp: new Date(), checksum,
    });
  ...
  verifyIntegrity(goalId: string): boolean {
    ...
    return filtered.every(e => {
      const computed = createHash('sha256').update(JSON.stringify({ traceId: e.traceId, goalId: e.goalId, action: e.action, metadata: e.metadata })).digest('hex');
...
export class ExecutionTraceManager {
  ...
  finishSpan(spanId: string, traceId: string, goalId: string, operation: string, startTime: Date, status: 'OK' | 'ERROR'): TraceSpan {
    const endTime = new Date();
    const durationMs = endTime.getTime() - startTime.getTime();
    const checksum = createHash('sha256').update(JSON.stringify({ traceId, goalId, operation, spanId, durationMs })).digest('hex');
    const span: TraceSpan = Object.freeze({
      spanId, traceId, goalId, operation, startTime, endTime, durationMs, status, checksum,
    });
  ...
  validateTrace(traceId: string): boolean {
    return this.getSpans(traceId).every(s => {
      const computed = createHash('sha256').update(JSON.stringify({ traceId: s.traceId, goalId: s.goalId, operation: s.operation, spanId: s.spanId, durationMs: s.durationMs })).digest('hex');

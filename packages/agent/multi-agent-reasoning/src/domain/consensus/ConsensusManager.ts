...
export class ConsensusManager {
  ...
  castVote(roundId: string, agentId: string, vote: boolean): void {
    ...
    if (!round) throw new ConsensusProtocolError(`Round not found: ${roundId}`, 'ROUND_NOT_FOUND', 'consensus-manager');
    if (!(agentId in round.votes)) throw new ConsensusProtocolError(`Agent not in round: ${agentId}`, 'AGENT_NOT_IN_ROUND', 'consensus-manager');

...
export class DistributedAuditManager {
  ...
  log(traceId: string, nodeId: string, sessionId: string, action: string, metadata: Record<string, unknown>): AuditEntry {
    const entryId = `da-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
    const checksum = createHash('sha256').update(JSON.stringify({ traceId, nodeId, sessionId, action, metadata })).digest('hex');

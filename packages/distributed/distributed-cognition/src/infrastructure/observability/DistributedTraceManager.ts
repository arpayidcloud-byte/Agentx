...
export class DistributedTraceManager {
  ...
  startSpan(_traceId: string, _nodeId: string, _operation: string, _parentSpanId: string | null = null): string {
    return `span-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
  }

  finishSpan(spanId: string, traceId: string, nodeId: string, operation: string, startTime: Date, parentSpanId: string | null, status: 'OK' | 'ERROR' | 'TIMEOUT'): TraceSpan {
    const endTime = new Date();
    const durationMs = endTime.getTime() - startTime.getTime();
    const checksum = createHash('sha256').update(JSON.stringify({ traceId, nodeId, operation, spanId, durationMs })).digest('hex');

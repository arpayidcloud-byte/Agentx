...
export class DistributedScheduler {
  ...
  constructor(policy: SchedulePolicy = { strategy: 'LEAST_LOADED', maxRetries: 3, timeoutMs: 30000 }) {
    this.policy = Object.freeze({ ...policy });
  }

  enqueue(goalId: string, priority: number, metadata: Record<string, unknown> = {}): DistributedTask {
    const taskId = `dt-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
    const checksum = createHash('sha256').update(JSON.stringify({ taskId, goalId, priority })).digest('hex');
  ...
  assign(taskId: string, nodeId: string): DistributedTask {
    ...
    const updated: DistributedTask = Object.freeze({ ...task, assignedNode: nodeId, state: 'ASSIGNED' });

 ...
 */
import { ApprovalNotFoundError, ApprovalAlreadyProcessedError, ApprovalExpiredError, ApprovalDoubleConfirmationRequiredError, ApprovalValidationError, } from './errors.js';
...
import { createApprovalCreatedEvent, createApprovalApprovedEvent, createApprovalRejectedEvent, createApprovalCancelledEvent, createApprovalExpiredEvent, createApprovalExecutedEvent, } from './approval-events.js';
/**
...
export class ApprovalEngine {
    ...
    /** @inheritdoc */
    async approve(requestId, operatorId, confirmed = true) {
    ...
    async execute(requestId) {
        ...
        if (request.state !== 'APPROVED') {
            throw new ApprovalValidationError(`Cannot execute request '${requestId}' in state '${request.state}'`);
        }
        // Mark as executed

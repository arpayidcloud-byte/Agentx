...
export class ShellExecutor {
    ...
    async execute(request) {
        ...
        try {
            ...
            cleanup();
            // 10. Validate output
            validateOutputSize(Buffer.byteLength(output.stdout) + Buffer.byteLength(output.stderr), resourceLimits);
            validateExecutionTime(output.durationMs, resourceLimits);
            // 11. Determine if timed out
            const timedOut = controller.signal.aborted;
            // 12. Emit tool.finished event
            this.auditEmitter.emit(createToolFinishedEvent(request.command, category, output.exitCode, output.durationMs, output.stdout.length, output.stderr.length, request.taskId, request.traceId, request.agentRole, timedOut));
            return {

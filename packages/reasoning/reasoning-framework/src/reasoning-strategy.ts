...
export class ReasoningStrategyBase implements IReasoningStrategy {
  ...
  async execute(graph: ReasoningGraph): Promise<ReasoningGraph> { return graph; }
  async validate(_graph: ReasoningGraph): Promise<boolean> { return true; }
  async checkpoint(_sessionId: string, _snapshot: Record<string, unknown>): Promise<void> {}
  async recover(_sessionId: string): Promise<Record<string, unknown>> { return {}; }

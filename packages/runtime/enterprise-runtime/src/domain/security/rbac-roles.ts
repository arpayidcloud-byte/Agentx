...
const ROLE_PERMISSIONS: Record<RBACRole, string[]> = {
  [RBACRole.OWNER]: [
    ...
    'task.execute', 'task.approve', 'task.reject',
    'graph.create', 'graph.read', 'graph.write', 'graph.delete',
    'plugin.install', 'plugin.enable', 'plugin.disable', 'plugin.list',
  ...
  [RBACRole.DEVELOPER]: [
    ...
    'task.execute',
    'graph.create', 'graph.read', 'graph.write',
    'plugin.list',
    'audit.read', 'cost.read',
  ],
  [RBACRole.VIEWER]: [

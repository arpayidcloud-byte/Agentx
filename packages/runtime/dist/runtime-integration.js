...
export class RuntimeIntegration {
    ...
    async executeGoal(sessionId, goal) {
        const pipeline = this.di.resolve('pipeline');
        return pipeline.execute({ id: sessionId, traceId: `trace-${Date.now()}`, goal });
    }
    async getHealthReport() {
        const healthService = this.di.resolve('healthService');
        return healthService.checkAll();
    }
    async getMetrics() {
        const observability = this.di.resolve('observability');
        return observability.getAggregatedMetrics();
    }
    getRecoveryHistory() {
        const recovery = this.di.resolve('recovery');

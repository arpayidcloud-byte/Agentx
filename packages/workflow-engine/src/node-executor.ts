...
export class NodeExecutor implements INodeExecutor {
  public async executeNode(node: WorkflowNode, _context: Record<string, unknown>): Promise<unknown> {
    switch (node.config.type) {
      ...
      case 'task': return { status: 'completed', type: 'task', goal: (node.config as any).goal };
      default: return { status: 'completed', type: node.config.type };
    }
  }

  public async executeToolNode(node: WorkflowNode, _context: Record<string, unknown>): Promise<unknown> {
    const config = node.config as { type: 'tool'; toolName: string; category: string; arguments: Record<string, unknown> };
  ...
  public async executeAgentNode(node: WorkflowNode, _context: Record<string, unknown>): Promise<unknown> {
    const config = node.config as { type: 'agent'; role: string; goal: string; context?: Record<string, unknown> };
  ...
  public async executeApprovalNode(node: WorkflowNode, _context: Record<string, unknown>): Promise<unknown> {
  ...
  public async executeParallelNode(node: WorkflowNode, _context: Record<string, unknown>): Promise<unknown> {
    const config = node.config as { type: 'parallel'; branches: string[] };
    const results: unknown[] = [];
    for (const branchId of config.branches) { results.push({ branchId, status: 'completed' }); }
    return { status: 'completed', type: 'parallel', branches: config.branches, results };
  }

  public async executeLoopNode(node: WorkflowNode, _context: Record<string, unknown>): Promise<unknown> {
    const config = node.config as { type: 'loop'; iterator: string; maxIterations: number; body: string[] };
    const iterations: unknown[] = [];
    for (let i = 0; i < Math.min(config.maxIterations, 10); i++) { iterations.push({ iteration: i, status: 'completed' }); }
    return { status: 'completed', type: 'loop', iterations: iterations.length };
  }

  public async executeConditionalNode(node: WorkflowNode, _context: Record<string, unknown>): Promise<unknown> {
    const config = node.config as { type: 'conditional'; condition: string; trueBranch: string; falseBranch: string };
